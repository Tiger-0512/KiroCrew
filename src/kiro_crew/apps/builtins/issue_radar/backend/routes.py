...
async def _handle_deps(request: web.Request) -> web.Response:
    ...
    Returns ``{owner, repo, provider, host, edges, nodes, from_cache, stale}``
    where an edge is ``{blocked, blocker, source: "native"|"inferred"}`` and
    ``nodes`` maps every number appearing in an edge to ``{kind, state, title}``.
    ...
    IMMEDIATELY with ``stale: true`` while a single coalesced background refresh
    rebuilds it off the request path — the ~11s rebuild never blocks a user. Only
    a genuinely never-synced repo (no cache at all) still blocks on a build.
    ``refresh=1`` forces a SYNCHRONOUS rebuild so a user-initiated refresh returns
    fresh data. A rebuild reads the open issues (the graph's scope) plus the pulls
    cache (node hints) and syncs the native + inferred edges via
    ``github_client.fetch_dependency_edges``.

    ``stale`` is additive: it is ``false`` on a fresh or freshly-built response
    and ``true`` only on a served-stale one, so a client that ignores it keeps
    working on the unchanged ``edges``/``nodes``/``from_cache`` shape.
