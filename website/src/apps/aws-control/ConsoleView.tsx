 ...
 * drive (spec §3): a stats strip, then either a setup card (when the bucket does
 * not exist) or the drive's Library / Drive / Backup / Access sections, plus the
 * still-dashed Tasks and Sites app ghosts. Every mutation here is confirmed
 * before it runs and ends by invalidating its react-query key. All AWS access
 * runs through the gateway's audited CLI chokepoint — this surface never talks
 * to AWS from the browser.
 */
...
import {
  ...
  FolderClosed, FileText, X, MoreHorizontal,
} from 'lucide-react'
...
/** Human-readable last four of the account id, for the crumb + header. */
...
function DriveSectionView({ account, bucket }: { account: string; bucket: string }) {
  ...
  const uploadMut = useMutation({
    mutationFn: (file: File) =>
      awsControlApi.driveUpload(account, 'drive', `${path}${file.name}`, file),
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="px-4 pt-2 pb-3 md:px-6">
        ...
        >
          <ChevronLeft size={14} />
          {i18nT('apps.awsControl.console.crumb_accounts')} / <span className="font-mono text-text">···{tail}</span>
        </button>
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-lg text-muted">···<span className="text-text-strong font-semibold">{tail}</span></span>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto px-4 pb-6 md:px-6">
        {/* Section 2: stats */}
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4" data-testid="console-stats">
