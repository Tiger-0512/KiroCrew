...
export interface PierrePerfWindow {
  ...
  keys: number
  /** Distinct keys minted for the single busiest RENDERED SURFACE — the churn
   *  verdict.
   *
   *  The attribution unit is a surface (a diff's old side, its new side, an
   *  editor's base, a standalone block), not a filename, because a filename is
   *  not a tokenization unit: a diff's two sides share one name yet legitimately
   *  tokenize once each, so a per-name count reads every settled diff as 2 — a
   *  false confirm. Per surface, everything that renders once stays at 1 and
   *  only a streamed surface climbs to its chunk count: one block streamed in 5
   *  chunks reaches 5 here, five independent blocks stay at 1 each, and a
   *  settled diff is 1 per side. Confirming the re-tokenize hypothesis reads
   *  THIS field, not the ratio. */
 ...
 *
 *  `surface` is the attribution identity: the caller qualifies the file name with
 *  the rendered surface (diff side, editor base, standalone block), because a
 *  bare filename conflates a diff's two sides into false churn. */
export function recordCacheKey(surface: string, key: string, length: number): void {
  if (!enabled) return
  calls += 1
  chars += length
  if (length > maxLen) maxLen = length
  let seen = keysBySurface.get(surface)
  if (!seen) {
    seen = new Set<string>()
    keysBySurface.set(surface, seen)
  }
