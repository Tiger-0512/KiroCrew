 ...
 * line being typed.
 *
 * `active` mirrors the hosting tab's visibility. It exists for the async key
 * (Paste): a clipboard read can take seconds on iOS (the permission callout),
 * and a paste that resolves after the user switched tabs must be DROPPED, not
 * delivered — `term` still points at the now-hidden terminal, and a
 * newline-terminated clipboard would execute in a shell the user cannot see.
 */
type PasteError = 'paste_failed' | 'paste_clipboard_empty' | 'paste_permission_needed'
...
export default function TerminalKeyBar({ term, active = true }: { term: Terminal; active?: boolean }) {
  ...
  const handlePaste = () => {
    ...
    // write-only clipboard support.
    const read = navigator.clipboard?.readText?.bind(navigator.clipboard)
    if (!read) { fail('paste_failed'); return }
    busyRef.current = true
    read()
      .then(text => {
        // Staleness gate: between the tap and the clipboard resolving, the
        // user may have switched tabs (or closed this one). Delivering then
        // would type into a hidden shell — drop instead.
        if (!aliveRef.current || !activeRef.current) { busyRef.current = false; return }
    ...
    >
      ...
      <div className="flex shrink-0 items-center border-l border-border pl-2">
        <button
          ...
          onPointerDown={e => e.preventDefault()}
          onClick={handlePaste}
          className="flex min-w-[2.25rem] max-w-[70vw] shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-md border border-border bg-bg-elevated px-2 py-1.5 font-mono text-[13px] text-text active:bg-bg-hover"
        >
