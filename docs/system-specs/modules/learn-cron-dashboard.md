...
transport action starts. Dashboard, Slack, and Discord receive the same ephemeral,
...
rehydrating the result when no WebSocket is available. While disconnected, each
successful refreshed slot snapshot is reconciled into the shared Redux collection,
including an empty-result tombstone, so an older WebSocket value cannot mask a
completed update or stop.
